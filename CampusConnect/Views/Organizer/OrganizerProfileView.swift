    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 24) {
                ...
                // Info rows
                VStack(spacing: 0) {
                    infoRow(icon: "building.2.fill", color: Constants.Colors.accent, label: "Organization", value: organizer.department)
...
struct OrganizerContactView: View {
    ...
    private func contactButton(icon: String, title: String, value: String, color: Color) -> some View {
        let sanitizedPhone = value.filter { $0.isNumber || $0 == "+" }
        let destination: URL? = {
            if title == "Email" {
        ...
        return Group {
            if let destination {
                Link(destination: destination) {
                    contactRow(icon: icon, title: title, value: value, color: color)
                }
            } else {
                contactRow(icon: icon, title: title, value: value, color: color)
            }
