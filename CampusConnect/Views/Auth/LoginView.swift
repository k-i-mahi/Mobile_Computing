...
struct LoginView: View {
    ...
    var body: some View {
        NavigationStack {
                ...
                ScrollView(showsIndicators: false) {
                    VStack(spacing: 0) {
                        ...
                        VStack(spacing: 20) {
                            ...
                            Text("Campus-only access: use your @\(Constants.campusEmailDomain) email.")
                                .font(.caption)
                            ...
                            if !isSignUpMode {
                                Button("Forgot Password?") {
                                    Task {
                                        let canProceed = await authViewModel.preparePasswordReset(email: email)
                                        if canProceed {
                                            resetTargetEmail = email.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
            ...
            .navigationDestination(isPresented: $showResetPasswordView) {
                ResetPasswordView(email: resetTargetEmail)
            }
        }
        .onAppear {
    ...
    private func errorBanner(_ message: String) -> some View {
        let style = bannerStyle(for: message)
    ...
    private func bannerStyle(for message: String) -> (icon: String, foreground: Color, background: Color) {
        let lower = message.lowercased()
        if lower.contains("success") || lower.contains("created successfully") {
            return ("checkmark.circle.fill", Constants.Colors.success, Constants.Colors.success.opacity(0.1))
        }
        return ("exclamationmark.circle.fill", Constants.Colors.danger, Constants.Colors.danger.opacity(0.08))
    }
